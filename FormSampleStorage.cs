    ...
    public partial class FormSampleStorage : Form
    {
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (DB.NameExists(connection, transaction, "sample_storage", p["name"].ToString(), SampleStorageId))
                {
                    MessageBox.Show("The sample storage '" + p["name"] + "' already exists");
                    return;
                }

                if (!p.ContainsKey("id"))
                    InsertSampleStorage(connection, transaction);
                else
                    UpdateSampleStorage(connection, transaction);
        ...
        private void InsertSampleStorage(SqlConnection conn, SqlTransaction trans)
        {            
            ...
            SqlCommand cmd = new SqlCommand("csp_insert_sample_storage", conn, trans);
            cmd.CommandType = CommandType.StoredProcedure;
            p["id"] = Guid.NewGuid();
            cmd.Parameters.AddWithValue("@id", p["id"]);
        ...
        private void UpdateSampleStorage(SqlConnection conn, SqlTransaction trans)
        {            
            p["update_date"] = DateTime.Now;
            p["updated_by"] = Common.Username;        

            SqlCommand cmd = new SqlCommand("csp_update_sample_storage", conn, trans);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@id", p["id"]);
            cmd.Parameters.AddWithValue("@name", p["name"]);
            cmd.Parameters.AddWithValue("@address", p["address"], String.Empty);
